 ...
 * Uses FromFedToChain's ContentPipelineService via dynamic import.
 *
...
async function main() {
    ...
    try {
        ...
        const servicePath = path.join(fromFedToChainPath, 'src/services/ContentPipelineService.js');

        const { ContentPipelineService } = await import(servicePath);

        if (!ContentPipelineService || typeof ContentPipelineService.uploadContentToCloudflare !== 'function') {
            throw new Error('ContentPipelineService.uploadContentToCloudflare is not available');
        }
        ...
        console.error('Content upload failed:', error instanceof Error ? error.message : error);
        process.exit(1);
    }
}
